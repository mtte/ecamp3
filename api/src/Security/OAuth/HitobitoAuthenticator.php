...
class HitobitoAuthenticator extends OAuth2Authenticator {
    ...
    public function supports(Request $request): ?bool {
        ...
        return 1 === preg_match('/^connect_(pbsmidata|cevidb|jubladb|jemkdb)_check$/', $request->attributes->get('_route'));
    }

    public function authenticate(Request $request): Passport {
        // extract provider from request path
        preg_match('/^\/auth\/(pbsmidata|cevidb|jubladb|jemkdb)\/callback$/', $request->getPathInfo(), $providerMatch);
